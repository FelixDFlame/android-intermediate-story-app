...
class StoryItemRemoteMediator(
    ...
    override suspend fun load(
        ...
        try {
            val responseData = apiService.getCollectionStoryCallback(
                auth = "Bearer $token",
                location = 0,
                page = page,
            ...
            val responseListStory = responseData.listStory

            val endOfPaginationReached = responseListStory.isEmpty()
            database.withTransaction {
                ...
                val keys = responseListStory.map {
                    RemoteKeys(id = it.id, prevKey = prevKey, nextKey = nextKey)
                }
                database.remoteKeysDao().insertAll(keys)
                database.storyDao().insertStories(responseListStory)
            }
